...
import { CheckupItem } from '../../api/checkups';
...
const translations = {
  ru: {
    ...
    appointmentError: 'Ошибка при записи на прием. Пожалуйста, попробуйте позже.',
  ...
  uz: {
    ...
    appointmentError: 'Qabulga yozilishda xatolik yuz berdi. Iltimos, keyinroq urinib ko\'ring.',
...
export default function DynamicAppointmentBooking() {
  ...
  const [loadingSpecializations, setLoadingSpecializations] = useState<boolean>(false);
  ...
  const handleServiceBooking = async () => {
    ...
    try {
      ...
      if (selectedService === 'analysis') {
        ...
        const selectedAnalysis = analyses.find(item => 
        ...
        const selectedCheckup = checkups.find(item => 
      ...
      if (error instanceof Error) {
        ...
        const errorMessage = error.response?.data?.message || t('appointmentError');
        alert(errorMessage);
      } else {
        alert(t('appointmentError'));
      }
  ...
  if (isBooked && bookingDetails) {
    ...
    const getServiceLabel = () => {
      const serviceOption = services.find(s => s.value === bookingDetails.service);
    ...
    return (
      <AppointmentConfirmation 
        ...
        doctor={bookingDetails.service === 'doctor' ? bookingDetails.name : undefined}
    ...
    <div className="">
      <div className={`${cardBg} rounded-2xl p-4 md:p-8 w-full mx-auto`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6">
          ...
          <div>
            <h2 className={`text-lg md:text-xl font-medium mb-2 md:mb-4 ${textColor}`}>
              {selectedService === 'doctor' ? t('selectSpecialty') : 
               selectedService === 'analysis' ? t('selectAnalysisType') : 
               selectedService === 'checkup' ? t('selectCheckupProgram') : 
               t('selectSpecialty')}
            </h2>
            <div className="relative" ref={specialtyDropdownRef}>
              <button
                ...
                className={`
                  ...
                  focus:outline-none
                  ${borderColor} ${inputBg} ${textColor}
                  ${(!selectedService || loading || loadingSpecializations) ? 'opacity-50 cursor-not-allowed' : ''}
                `}
              >
                {loadingSpecializations ? t('loadingSpecializations') : 
                 loading ? t('loading') : 
                 selectedSpecialty || (selectedService === 'doctor' ? t('selectSpecialty') : 
                                    selectedService === 'analysis' ? t('selectAnalysisType') : 
                                    selectedService === 'checkup' ? t('selectCheckupProgram') : 
                                    t('selectSpecialty'))}
                <ArrowDownIcon 
              ...
              {isSpecialtyDropdownOpen && selectedService && !loading && !loadingSpecializations && (
                <div className={`
                  absolute z-20 mt-1 w-full
                  rounded-xl shadow-lg border 
