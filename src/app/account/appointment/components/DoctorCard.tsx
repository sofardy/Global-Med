...
export const DoctorCard: React.FC<DoctorCardProps> = ({ 
  ...
  const borderColor = theme === 'light' ? 'border-[#94A3A6]' : 'border-gray-700';
  const inputBgColor = theme === 'light' ? 'bg-white' : 'bg-dark-bg';
  const inputBorderColor = theme === 'light' ? 'border-gray-200' : 'border-gray-700';
  const accentColor = 'ring-[#00C78B]';
  
  return (
    <div className={`${cardBg} rounded-2xl shadow-sm ${borderColor} border overflow-hidden mb-6`}>
      ...
      <div className="p-3 sm:p-4 md:p-8">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="flex-1">
            <h3 className={`text-lg sm:text-xl md:text-2xl font-medium ${textColor}`}>
              {doctor.name}
            </h3>
            ...
            <ul className="mb-3 md:mb-6 space-y-1 md:space-y-2">
              <li className={`flex items-start ${mutedTextColor} text-xs sm:text-sm md:text-base`}>
                <span className={`${mutedTextColor} mr-2`}>•</span>
                <span>{t('experiencePrefix')} {doctor.experience} {t('experienceSuffix')}</span>
              </li>
              <li className={`flex items-start ${mutedTextColor} text-xs sm:text-sm md:text-base`}>
                <span className={`${mutedTextColor} mr-2`}>•</span>
      ...
      <div className={`p-3 sm:p-4 md:p-8 ${cardBg} border-t ${borderColor}`}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-3 md:gap-4 items-end">
          {/* Date Selection */}
          <div>
            <div className={`mb-1 md:mb-2 ${textColor} text-xs sm:text-sm md:text-base`}>{t('selectDate')}</div>
            <input
              type="date"
              value={selectedDate}
          ...
          {/* Time Selection */}
          <div>
            <div className={`mb-1 md:mb-2 ${textColor} text-xs sm:text-sm md:text-base`}>{t('selectTime')}</div>
            <select
              value={selectedTime}
