...
export default function ServiceDetail({ params }: ServiceDetailProps) {
  ...
  // Normalize symptoms_list to an array of strings
  const symptoms = serviceData.symptoms_list.map(symptom =>
      typeof symptom === 'string' ? symptom : (symptom && 'name' in symptom ? symptom.name : '')
  );

  // Displayed symptoms based on state
  const displayedSymptoms = isMobile && !expandedSymptoms
      ? symptoms.slice(0, visibleSymptomsCount)
      : symptoms;

  // Displayed services based on state
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8 md:mb-16">
          ...
          <div className={`rounded-2xl p-6 sm:p-8 ${theme === 'light' ? 'bg-white' : 'bg-dark-block'} relative overflow-hidden h-auto md:h-[700px]`}>
            ...
            <div className="relative z-10">
              ...
              {isMobile && symptoms.length > visibleSymptomsCount && (
                  <button
            ...
            <div className="relative z-10 overflow-x-auto">
              <div className="min-w-[280px]">
                {displayedServices.map((service, index) => (
                    <div
                        key={index}
                        className={`py-3 sm:py-4 flex justify-between items-center ${(isMobile && !expandedServices && index === displayedServices.length - 1) ||
