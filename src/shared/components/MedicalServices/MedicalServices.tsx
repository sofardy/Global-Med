...
export const MedicalServices = () => {
  ...
  const displayedServices = isMobile && !showAllItems ? services.slice(0, 4) : services;
                  ...
                  {displayedServices.length > 0 ? displayedServices.map((service) => (
                    <div key={service.uuid} className="h-full">
                      <UniversalCard
                        ...
                        additionalInfo={`${service.services_list?.length || 0} ${t('servicesCountLabel')}`}
                        icon={service.icon}
