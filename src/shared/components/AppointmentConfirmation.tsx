...
const translations = {
  ru: {
    ...
    baseInstruction:
      "Перед первым посещением клиники возьмите с собой удостоверение личности",
    additionalInstruction:
      "и актуальные медицинские анализы и обследования (при наличии)",
  ...
  uz: {
    ...
    baseInstruction:
      "Klinikaga birinchi tashrif buyurishdan oldin shaxsiy guvohnomangizni olib keling",
    additionalInstruction:
      "va dolzarb tibbiy tahlillar va tekshiruvlar (mavjud bo'lsa)",
...
export default function AppointmentConfirmation({
  ...
  const borderColor =
    theme === "light" ? "border-[#E2E8F0]" : "border-[#174F4B]";
  ...
  return (
    <div className={`${cardBg} rounded-2xl p-4 md:p-8 shadow-sm`}>
      <h1
        className={`text-xl md:text-2xl lg:text-3xl font-medium ${textColor} mb-4`}
      >
        {getTitle()}
      </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-6 md:mb-10">
        ...
        <div className={`border ${borderColor} rounded-xl p-3 md:p-4`}>
          <div className="flex items-center mb-1">
            <svg
              className={`w-4 h-4 md:w-5 md:h-5 mr-2 ${textColor}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <div
            className={`${textColor} text-base md:text-lg ml-6 md:ml-7 font-medium`}
          >
            {date}
          </div>
        </div>
        ...
        {/* Address */}
        <div
          className={`border ${borderColor} rounded-xl p-3 md:p-4 sm:col-span-2 md:col-span-1`}
        >
          <div className="flex items-center mb-1">
            <svg
              className={`w-4 h-4 md:w-5 md:h-5 mr-2 ${textColor}`}
          ...
          <div
            className={`${textColor} text-base md:text-lg ml-6 md:ml-7 font-medium line-clamp-2`}
          >
            {address}
          </div>
        </div>
