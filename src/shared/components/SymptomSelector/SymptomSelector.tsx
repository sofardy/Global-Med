...
const translations = {
  ru: {
    ...
    subtitle:
      "Выберите свой симптом — и мы подскажем, к какому специалисту записаться, какие анализы помогут уточнить диагноз, а также какие чек-апы стоит пройти для комплексной проверки здоровья",
    ...
    recommendedSpecialist:
      "Рекомендуем консультацию специалиста по выбранным симптомам",
    seeMore: "Подробнее",
    andMore: "и еще",
    moreTests: "анализов",
    symptoms: [
  ...
  uz: {
    ...
    subtitle:
      "O'z simptomingizni tanlang — va biz qaysi mutaxassisga yozilishni, qaysi tahlillar tashxisni aniqlashtirish uchun yordam berishini, shuningdek sog'liqni kompleks tekshirish uchun qanday tekshiruvlardan o'tish kerakligini aytib beramiz",
    ...
    recommendedSpecialist:
      "Tanlangan simptomlar bo'yicha mutaxassis maslahatini tavsiya qilamiz",
    seeMore: "Batafsil",
    andMore: "va yana",
    moreTests: "tahlillar",
    symptoms: [
...
export const SymptomSelector: React.FC = () => {
  ...
  const [checkupData, setCheckupData] = useState<CheckupsApiResponse | null>(
    null
  );
  ...
  const getSymptomSlug = (symptom: string): string => {
    return (
      symptomSlugs[symptom] ||
      symptom.toLowerCase().replace(/\s+/g, "-").replace(/ё/g, "e")
    );
  };
  ...
  const fetchCheckupData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/checkups?${symptomsParams}`
      );
  ...
  const generateCardsFromApiData = (): CardData[] => {
    ...
    if (
      !checkupData ||
      loading ||
      (checkupData.data && checkupData.data.length === 0)
    ) {
    ...
    const limitedCheckups =
      checkupTitles.length > maxCheckups
        ? [
            ...checkupTitles.slice(0, maxCheckups),
            `...${t("andMore")} ${checkupTitles.length - maxCheckups}`,
          ]
        : checkupTitles;

    // Ограничиваем список анализов
    const limitedTests =
      testNames.length > maxTests
        ? [
            ...testNames.slice(0, maxTests),
            `...${t("andMore")} ${testNames.length - maxTests} ${t(
              "moreTests"
            )}`,
          ]
        : testNames;
    ...
    const limitedSymptoms =
      relatedSymptomsList.length > maxSymptoms
        ? [
            `${t("relatedSymptoms")} ${relatedSymptomsList
              .slice(0, maxSymptoms)
              .join(", ")}...`,
          ]
        : relatedSymptomsList.length > 0
        ? [`${t("relatedSymptoms")} ${relatedSymptomsList.join(", ")}`]
        : [];

    // Ограничиваем список сервисов (чек-апов с ценой и длительностью)
    const serviceInfo = checkupData.data
      .slice(0, 2)
      .map((c) => `${c.title}: ${c.duration} мин, ${c.price} UZS.`);
    ...
    return [
      ...
      },
      {
        ...
        subtitle: selectedSymptoms.join(", "),
        description: [...limitedSymptoms, ...serviceInfo],
      ...
      },
      {
        ...
        description:
          limitedSymptoms.length > 0
            ? limitedSymptoms[0]
            : (t("recommendedSpecialist") as string),
  ...
  const toggleTempSymptom = (symptom: string) => {
    if (tempSelectedSymptoms.includes(symptom)) {
      setTempSelectedSymptoms(
        tempSelectedSymptoms.filter((s) => s !== symptom)
      );
