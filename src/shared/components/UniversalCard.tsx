...
export const UniversalCard: React.FC<UniversalCardProps> = ({
	...
	const getAnimationClass = () => {
		...
		switch (animation) {
			...
			default:
				return animation.startsWith("animate-") ? animation : `animate-${animation}`;
		}
	};
  ...
  `;
	...
	const titleSizeClass = isMobile
		...
		: titleSize || "text-xl sm:text-2xl md:text-[20px] leading-tight sm:leading-normal md:leading-[1.2]";
						...
						<div className="flex items-center">
							<svg
								className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 mr-2 ${
									isHovered ? "text-white" : theme === "light" ? "text-light-text" : "text-dark-text"
								}`}
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							...
							<span className={`text-sm sm:text-base ${isHovered ? "text-white" : ""}`}>
	...
	const AdditionalInfoBlock = () =>
        ...
        ${isHovered ? "text-white" : theme === "dark" ? "text-[#FFFFFF80]" : "text-[#094A5480]"} 
	...
	const renderFeatures = (features: Array<string | Feature> | undefined, isHovered: boolean, theme: string) => {
	...
	const IconBlock = () => {
		...
		if (typeof icon === "string") {
			...
			return (
				<div className={iconClasses} style={{ color: isHovered ? "white" : finalIconColor }}>
					<div className={"w-[190px] h-[190px] flex items-center"} dangerouslySetInnerHTML={{ __html: modifiedSvg }} />
				</div>
			);
		} else {
				...
				<div className={iconClasses} style={styles.icon}>
					{typeof icon === "string" ? (
						<div
							...
							className={`w-[190px] h-[190px] transition-transform duration-300 ${isHovered ? "scale-110" : ""}`}
							style={{ color: isHovered ? "white" : finalIconColor }}
						/>
					) : React.isValidElement(icon) ? (
						React.cloneElement(icon as React.ReactElement, {
							size: isMobile ? 133 : 190, // 190 * 0.7 for mobile
							className: `${(icon as React.ReactElement).props.className || ""} transition-transform duration-300 ${
								isHovered ? "scale-110" : ""
							}`,
							color: isHovered ? "white" : undefined,
						})
	...
	const SubtitleBlock = () =>
		subtitle && <h4 className={`font-medium ${descriptionSizeClass} mb-2 ${titleColor}`}>{subtitle}</h4>;
		...
		return (
			...
			>
				<div className="flex flex-col justify-between h-full">
					{/* Заголовок вверху */}
					<h3 className={`${titleSizeClass} font-medium ${titleColor}`}>{title}</h3>
					...
					<Link href={link || "#"} className="w-full block mt-4">
						<button
							className={`w-full ${buttonPadding || "py-3"} px-4 rounded-2xl font-medium transition-colors ${
			...
			>
				...
				<div
					className={`absolute top-2 right-2 w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3 rounded-full transition-colors duration-300 ${
						isHovered ? "bg-white" : theme === "light" ? "bg-[#F7F7F7]" : "bg-[#11363C]"
					}`}
				></div>
				...
				<div className="flex flex-col h-[280px] sm:h-[300px] md:h-[320px] relative">
					...
					{/* Features list */}
					<div className="mb-4 sm:mb-5">{renderFeatures(features, isHovered, theme)}</div>
		...
		<div className="flex flex-col h-full ">
			...
			{badge && (
				<div className="absolute top-3 sm:top-4 left-3 sm:left-4 z-10">
					{typeof badge === "string" ? (
						<span className="px-2 py-1 text-xs font-medium rounded-md bg-light-accent text-white">{badge}</span>
					) : (
						badge
					)}
				</div>
