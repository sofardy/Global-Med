...
const translations = {
  ru: {
    ...
    description: 'Мы объединяем передовые медицинские технологии, комфортные условия и команду опытных специалистов, чтобы обеспечивать качественное лечение каждому пациенту',
...
const gridDescription = description || t('description') || '';
  ...
  const gridBenefits = benefits || t('benefits', { returnObjects: true }) as any[];
  return (
  ...
  {gridTitle}
              ...
              <p className="text-base lg:text-[18px] mt-10 mb-12 tracking-normal leading-[160%] space-y-2">
                {gridDescription.split('. ').map((sentence, index) => (
                  <span key={index} className="block">{sentence}{index < gridDescription.split('. ').length - 1 ? '.' : ''}</span>
                ))}
              </p>
            </div>
            
          <a href={readMoreLink} className="relative z-10 h-[58px] inline-flex items-center justify-center border border-[#173F46] dark:border-white text-[#173F46] dark:text-white hover:bg-light-accent hover:text-white hover:border-light-accent rounded-2xl px-6 py-3 transition-colors mt-auto self-start">
  ...
  <svg className="ml-2 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path>
          ...
          <div className="col-span-1 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {Array.isArray(gridBenefits) && gridBenefits.map((benefit) => {
              return (
                <div key={benefit.id} className="h-full">
                  <UniversalCard
                    ...
                    showButton={false}
                    className={`${benefit.isActive ? 'bg-light-accent text-white' : ''}`}
                    iconPosition="center"
                        //   listStyle="disc"
                          minHeight="h-[360px]"
