...
const InfoCard = ({ title, description, hasDot = false, isWide = false, isNumeric = false, className = '' }) => {
  ...
  return (
    <div
      className={`relative p-4 sm:p-5 md:p-6 rounded-2xl transition-colors duration-300 ${isHovered
        ? 'bg-light-accent text-white'
        : theme === 'light' ? 'bg-white' : 'bg-dark-block'
        } ${isWide ? 'col-span-2' : 'col-span-1'} h-auto ${isWide
    ...
    >
      {hasDot && (
        <div
          className={`absolute top-2 sm:top-3 right-2 sm:right-3 w-2 h-2 sm:w-3 sm:h-3 md:w-4 md:h-4 rounded-full transition-colors ${isHovered
            ? 'bg-white'
      ...
      <div className="h-full flex flex-col">
        <h3 className={`${isWide
          ...
          } font-semibold mb-1 sm:mb-2 transition-colors leading-tight ${isHovered
            ? 'text-white'
        ...
        <p className={`text-xs sm:text-sm md:text-base lg:text-lg mt-1 sm:mt-2 md:mt-3 transition-colors line-clamp-3 sm:line-clamp-4 md:line-clamp-none ${isHovered
          ? 'text-white'
