...
export const localization = {
	en: {
		contactForm: {
			...
			subtitle: "And we will help you choose an individual solution for your corporate health",
			name: "Name",
			phone: "Phone",
			...
			successSubtitle: "Our clinic specialists will contact you shortly",
			confirmationTitle: "Application Confirmation",
			confirmationMessage: "Please confirm your application submission. Our specialists will contact you shortly.",
...
export default function ContactForm({ title, subtitle }: ContactFormProps): JSX.Element {
		...
		<>
			<div id="contact-info-section" className={`w-full flex flex-col md:flex-row gap-5 mt-8 overflow-hidden`}>
				<div className={`p-6 md:p-8 rounded-2xl ${theme === "dark" ? "bg-dark-block" : "bg-light-block"} md:w-1/4`}>
					<div>
						...
						<p className="text-light-text/80 dark:text-dark-text/80">{subtitle || t("contactForm.subtitle")}</p>
					</div>
				</div>
				...
				<div className={`p-6 md:p-8 rounded-2xl ${theme === "dark" ? "bg-dark-block" : "bg-white"} md:w-1/2`}>
					<form onSubmit={handleSubmit}>
						<div className="mb-6">
							...
							/>
							{formErrors.name && <p className="mt-1 text-sm text-red-500">{t("contactForm.nameError")}</p>}
						</div>
							...
							<div className="relative">
								...
								<input
									...
									className={`w-full p-5 pl-16 text-lg rounded-xl bg-light-bg dark:bg-dark-bg border ${
										formErrors.phone ? "border-red-500" : "border-gray-200 dark:border-gray-700"
									} text-light-text dark:text-dark-text focus:outline-none focus:border-light-accent`}
									disabled={isSubmitting}
								/>
							</div>
							{formErrors.phone && <p className="mt-1 text-sm text-red-500">{t("contactForm.phoneError")}</p>}
						</div>
						...
						<div className="flex items-start mb-10">
							...
							<label
								...
								className={`ml-3 text-base font-medium ${
									formErrors.consent ? "text-red-500" : "text-gray-600 dark:text-gray-300"
								}`}
							>
			...
			<Modal isOpen={isConfirmationModalOpen} onClose={closeConfirmationModal}>
				...
				<div className="flex justify-between">
					<button onClick={closeConfirmationModal} className="btn btn-secondary">
						{t("contactForm.cancelButton")}
					</button>
					<button onClick={confirmSubmission} className="btn btn-primary">
						{t("contactForm.confirmButton")}
					</button>
				</div>
			</Modal>
