...
import { LocationIconk2, LogoIcon, LogoTextIcon, WebsiteIcon } from '@/src/shared/ui/Icon';
  ...
  return (
    <header className="w-full">
      <div className={`max-w-[1560px] rounded-2xl ${theme === 'light' ? 'bg-light-block' : 'bg-dark-block'} mx-auto h-[70px] sm:h-[90px] px-4 sm:px-[40px] py-[14px] flex justify-between items-center`}>
        ...
        >
          <LocationIconk2 size={20} className="sm:w-[24px] sm:h-[24px]" color="white" />
          <span className="text-[14px] sm:text-[18px] whitespace-nowrap">{t('backToMainSite')}</span>
        </Link>
      </div>
    </header>
  );
